...
def sort_hands(card_bids: dict) -> list[dict]:
    ...
    for hand in card_bids:
        ...
        if countOf(value_count.values(), 5) == 1:
            ...
            if value_count.get('J') == 4 or value_count.get('J') == 1: # alternatively: if 'J' in value_count
                hands_by_kind['five_of_a_kind'] = hand
            else:
                hands_by_kind['four_of_a_kind'] = hand

        elif countOf(value_count.values(), 3) == 1 and countOf(value_count.values(), 2) == 1:
            if value_count.get('J') == 3 or value_count.get('J') == 2: # alternatively: if 'J' in value_count
                hands_by_kind['five_of_a_kind'] = hand
            else:
                hands_by_kind['full_house'] = hand

        elif countOf(value_count.values(), 3) == 1:
            if value_count.get('J') == 3 or value_count.get('J') == 1:
                hands_by_kind['four_of_a_kind'] = hand
            else:
                hands_by_kind['three_of_a_kind'] = hand

        elif countOf(value_count.values(), 2) == 2:
            if value_count.get('J') == 2:
                hands_by_kind['four_of_a_kind'] = hand
            elif value_count.get('J') == 1:
                hands_by_kind['full_house'] = hand
            else:
                hands_by_kind['two_pair'] = hand

        elif countOf(value_count.values(), 2) == 1:
            if value_count.get('J') == 2 or value_count.get('J') == 1:
                hands_by_kind['three_of_a_kind'] = hand
            else:
                hands_by_kind['one_pair'] = hand

        else: # countOf(value_count.values(), 1) == 5:
            if value_count.get('J') == 1:
                hands_by_kind['one_pair'] = hand
            else:
    ...
    # combined = hands_by_kind['three_of_a_kind'] + hands_by_kind['two_pair'] + hands_by_kind['one_pair']
    # combined = hands_by_kind['four_of_a_kind'] + hands_by_kind['two_pair'] + hands_by_kind['one_pair']
